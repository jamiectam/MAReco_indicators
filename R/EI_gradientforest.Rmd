...
# adjust data
```{r}
pressures_ess<-pressures_ess |> 
              select (-year)
indicators_ess<-indicators_ess |> 
              select(-year)
...
# run code for gfa functions (run after the universal gfa code)
```{r}
## USAGE EXAMPLE (for a region):
# result <- gradient_forest_main("4X", nafo_pres_4X, nafo_ind_4X, save_dir = "gf_results")

ess_long_result<-gradient_forest_main("ess", pressures_ess, indicators_ess, save_dir = "gf_results")

wss_long_result<-gradient_forest_main("wss", pressures_wss, indicators_wss, save_dir = "gf_results")

# All high-res PNGs and .RData files are now in the "data" folder you have all key outputs as .RData in "data", and results loaded in 'result' list.
```
# RUN THIS FIRST more universal gfa
```{r}
source("D:/WOrk projects/MAReco_indicators/R/gf_plot_utils.R")
...
if (!exists("spRatio")) {
  stop("GF plotting helpers not loaded. Please source gf_plot_utils.R")
}
  ...
  dat <- missForest(dat)$ximp #na.roughfix(dat)
...
## Option 1- Impute:
dat <- missForest(dat)$ximp #na.roughfix(dat) previously
#
