...
NA_index <- get_NA_index(inds)
NA_index_output <- pivot_wider(NA_index, names_from = col_index, values_from = fraction_NA)
names(NA_index_output)[1] = "SS_Area"
write.csv(NA_index_output, file = paste(export.filepath, "/fraction_NA_", area, ".csv", sep = ""),
          row.names = FALSE)
...
## calculate STRATA level indicators      
```{r}
path <- file.path(here())
...
# fishery-independent data
RV <- read.csv(paste(path, "/final_output/strat_notlengthbased_qadj_final.csv", sep = ""), 
               head = TRUE, sep = ",", stringsAsFactors = FALSE)
RV_length <- read.csv(paste(path, "/final_output/strat_lengthbased_qadj_final.csv", sep = ""), 
                      head = TRUE, sep = ",", stringsAsFactors = FALSE)

# commercial landings data *NO LANDINGS AT STRAT level*
# landings converted from tonnes to kg to be on the same scale as the RV data (required for the FP indicators)
land <- read.csv(paste(path, "/output/Landings/strat_land.csv", sep = ""), 
                 head = TRUE, sep = ",", stringsAsFactors = FALSE) %>% 
  mutate(CATCH = CATCH * 1000)

Length_Weight <- read.csv(paste(path, "/final_output/strat_LengthWeight.csv", sep = ""),
                          header = TRUE, sep = ",",
                          stringsAsFactors = FALSE) # read in length-at-weight data for STRATA
...
NA_index <- get_NA_index(inds)
NA_index_output <- pivot_wider(NA_index, names_from = col_index, values_from = fraction_NA)
names(NA_index_output)[1] = "strat_Area"
write.csv(NA_index_output, file = paste(export.filepath, "/fraction_NA_", area, ".csv", sep = ""),
          row.names = FALSE)
...
eco_indicators_strat <- left_join(inds_filtered, short_inds, by = c("YEAR", "ID")) 

write.csv(eco_indicators_strat, file = paste(export.filepath, "/eco_indicators_", area, ".csv", sep = ""),
          row.names = FALSE)

save(eco_indicators_strat, file=paste(export.filepath, "/eco_indicators_", area, ".Rdata", sep = ""))
...
CSV goes into the R shared drive R:\Science\BIODataSvc\IN\marea
